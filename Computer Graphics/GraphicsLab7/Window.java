...
public class Window {
    ...
    Window(int w,int h){
        ...
        window=glfwCreateWindow(w,h,"Lab7",0,0);
    ...
    void gameLoop() {
        double time,next,frameTime=0,passed;
        int fps=0;
        time=getTime();
    ...
    private void input(Camera camera,Cylinder c){
        if (glfwGetKey(window,GLFW_KEY_W)==1) camera.move(0,6,0);
        if (glfwGetKey(window,GLFW_KEY_A)==1) camera.move(-6,0,0);
        if (glfwGetKey(window,GLFW_KEY_S)==1) camera.move(0,-6,0);
        if (glfwGetKey(window,GLFW_KEY_D)==1) camera.move(6,0,0);
        if (glfwGetKey(window,GLFW_KEY_LEFT_CONTROL)==1) camera.move(0,0,-6);
        if (glfwGetKey(window,GLFW_KEY_SPACE)==1) camera.move(0,0,6);

        if (glfwGetKey(window,GLFW_KEY_X)==1) camera.rotate(2,0,0);
        if (glfwGetKey(window,GLFW_KEY_Y)==1) camera.rotate(0,2,0);
        if (glfwGetKey(window,GLFW_KEY_Z)==1) camera.rotate(0,0,2);
