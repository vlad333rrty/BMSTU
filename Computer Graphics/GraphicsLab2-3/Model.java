...
class Pyramid extends Model{
    protected double a,b,height,step=0.1,lineStep=20;
    protected boolean flag=false;
    ArrayList<Vector3d> side =new ArrayList<>();
    ArrayList<Vector3d> basement =new ArrayList<>();
    ArrayList<Vector3d> line=new ArrayList<>();

    Pyramid(Vector3f centre, double a,double b, double height) {
        this.step=PI*2/3;
        this.a=a;
        this.b=b;
        this.height=height;
    ...
    protected void calc(){
        ...
        for (angle=0;angle<pi2;angle+=step){
            x = cos(angle);
            y = sin(angle);
            side.add(new Vector3d(b*x,b*y,height));
            side.add(new Vector3d(a*x, a*y, -height));
        }
        side.add(new Vector3d(b, 0, height));
        side.add(new Vector3d(a, 0, -height));

        for (var i=-height+lineStep;i<height;i+=lineStep){
            double v = (i - height) / (-height - height);
            for (angle=0; angle<pi2; angle+=step){
                x = cos(angle);
                y = sin(angle);
                line.add(new Vector3d(v *(a*x-b*x)+b*x,y,i));
            }
            line.add(new Vector3d(v *(a-b)+b,0,i));
        }
